...
def open_file(event=None):
    global url 
    url=filedialog.askopenfilename(initialdir=os.getcwd(),title='Select title',filetypes=(('Text File','*.txt'),('All files',"*.*")))
    try:
...
view.add_checkbutton(label='Status Bar',onvalue=1,offvalue=0,image=status_bar_icon,compound=tk.LEFT,variable=show_statusbar,command=hide_statusbar)
#color theme
...
c=0
for i in color_dict:
    color_theme.add_radiobutton(label=i,image=color_icons[c],variable=theme_choice,compound=tk.LEFT,command=change_theme)
    c+=1
...
font_size.current(font_size.index(16))
